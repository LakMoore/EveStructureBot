...
export function generateCorpDetailsEmbed(thisCorp: AuthenticatedCorp) {
  ...
  const notificationTime =
    Math.round(NOTIFICATION_CHECK_DELAY / (6000 * directors.length)) / 10;
  const structureTime =
    Math.round(STRUCTURE_CHECK_DELAY / (6000 * stationManagers.length)) / 10;
  const posTime =
    Math.round(STRUCTURE_CHECK_DELAY / (6000 * directors.length)) / 10;

  const notificationMessage =
    directors.length == 0
      ? 'No directors found. Unable to check Notifications!'
      : `${directors.length} director${
          directors.length == 1 ? '' : 's'
        }; checking notifications every ${notificationTime} minute${
          notificationTime == 1 ? '' : 's'
        }.`;
  const structureMessage =
    stationManagers.length == 0
      ? 'No station managers found. Unable to check Structure Status!'
      : `${stationManagers.length} station manager${
          stationManagers.length == 1 ? '' : 's'
        }; checking stucture status every ${structureTime} minute${
          structureTime == 1 ? '' : 's'
        }.`;
  const posMessage =
    directors.length == 0
      ? 'No directors found. Unable to check POS Status!'
      : `${directors.length} director${
          directors.length == 1 ? '' : 's'
        }; checking POS status every ${posTime} minute${
          posTime == 1 ? '' : 's'
        }.`;
  ...
  if (chars.length < 10) {
    fields.push({
      ...
      value: `Recommend authorising at least ${
        10 - directors.length
      } more director${10 - directors.length == 1 ? '' : 's'}!`,
    });
  }
